...
Greffée depuis le projet frère VSCode2 via VSCode3 (2026-07-23). Complémentaire de :
...
3. **Transposer, pas copier** : adapter les cotes au gabarit du deck cible
   (ce projet : deck **binaire** OHC « dispositif d'écoute » dans `Exports/`,
   16:9 10×5.62 in, thème navy `#0E2356` / cyan `#00D2DD` / slate `#586586`,
   police Outfit, layouts natifs du template OCTO — pas de générateur maison,
   on édite la copie versionnée courante via des scripts python-pptx ad hoc)
   en respectant sa grammaire visuelle (label cyan uppercase + filet + puces
   navy, « chaque bloc à la taille de son contenu »).
4. **Vérifier au rendu réel** (`pptx-verify` + ouverture PowerPoint COM réelle,
   cf. mémoire projet) — un pattern bien choisi mais mal dimensionné reste un
   défaut.
