...
- **Jamais les JSONL bruts** : ne pas ouvrir `~/.claude/projects/.../*.jsonl` ni
  ...
  peuvent contenir du contenu métier sensible (documents internes, entretiens, etc.
  selon le projet) : analyse strictement locale.
...
| `verification-manquante` | Quelle vérification réelle est systématiquement sautée ? | `verifications_oubliees` ; commit touchant un export PPT sans passage `pptx-verify` |
