...
| --- | --- | --- |
| `dev-verifie` | Implémentation/correction avec tests + vérif réelle + revue-increment avant commit | Repris (éprouvé sur le projet frère VSCode2, pas encore rejoué ici) |
| `export-ppt-verifie` | Livrable = le deck de restitution : génération + enrichissements conditionnels (cadres photo, polish, design) + `pptx-verify` obligatoire | Repris (éprouvé sur le projet frère VSCode2, pas encore rejoué ici) |
| `revue-design-parallele` | Revue multi-angles d'un livrable en fan-out puis consolidation | Repris (éprouvé sur le projet frère VSCode2, pas encore rejoué ici) |
...
| --- | --- |
| Template/CSS/JS (UI web) | Screenshot via le skill `run` (pas seulement pytest) |
| Export PPT (python-pptx) | `pptx-verify` (rendu réel — python-pptx est un parseur tolérant) |
